...
def manage_pupils_reports():
    ...
    # Compute class-level stats per term (combined averages, positions)
    ...
    class_stats = {}  # term -> { class_average, class_rankings, pupil_positions }
    subject_count = len(subjects) if subjects else 0
    for term, exam_ids_in_term in term_groups.items():
        # build weights like other routes: Midterm 40% End_Term 60%
        ...
        # compute combined averages for each pupil in class
        combined_map = {}
        for p in pupils_in_class:
        ...
        # rankings within stream (for the pupil's stream)
