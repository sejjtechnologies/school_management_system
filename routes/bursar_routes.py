...
def student_receipt(pupil_id):
    ...
    try:
        ...
        if current_user and getattr(current_user, 'is_authenticated', False):
            ...
            elif hasattr(current_user, 'full_name') and current_user.full_name:
                cashier_name = current_user.full_name
            elif hasattr(current_user, 'first_name'):
                # combine first and last if available
                fn = getattr(current_user, 'first_name', '') or ''
                ln = getattr(current_user, 'last_name', '') or ''
                cashier_name = (fn + ' ' + ln).strip() or cashier_name
    except Exception:
        # if flask_login not installed or any other error, keep placeholder
        pass
